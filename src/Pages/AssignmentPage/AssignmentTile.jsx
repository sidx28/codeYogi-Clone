  ...
  return (
    <li className="w-full border-2 border-gray-100 bg-white rounded-lg shadow-lg mb-5 list-none " >
      <div className="flex flex-col items-center justify-between w-full p-3">
        <Link to={`/assignment/${assignmentsDetails[props.id].assignmentNumber}/details`} className="w-full">
          <div className="flex justify-between items-center flex-row">
            <div>
              <div className="flex flex-row">
                <h3 className="font-medium text-gray-900">#{assignmentsDetails[props.id].assignmentNumber} {assignmentsDetails[props.id].title}</h3>
                <span className="text-gray-500 font-semibold ml-3">( {assignmentsDetails[props.id].createdDate} )</span>
              </div>
              <p className="text-red-600 mt-3">Due Date: {assignmentsDetails[props.id].dueDate}</p>
            </div>
